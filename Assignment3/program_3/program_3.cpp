...
// Track window size and background texture size
static int gWinW = 800, gWinH = 600;
static int gBgTexW = 1, gBgTexH = 1;
static float gSkyUOffset = 0.25f; // shift the seam 90 degrees to the side

// Function to load a BMP file and return a texture ID
GLuint loadBMP(const char* filename) {
    ...
    // Save background texture size (used for aspect fit)
    gBgTexW = width; gBgTexH = height;

    // Some BMP files have imageSize set to 0
    ...
    // Upload texture to OpenGL (BMP is BGR format)
...
void init() {
    ...
    // Load the background texture - try multiple possible paths
    backgroundTexture = loadBMP("side.bmp");
    if (backgroundTexture == 0) {
        backgroundTexture = loadBMP("../side.bmp");
    }
    if (backgroundTexture == 0) {
        backgroundTexture = loadBMP("/Users/cookie/CLionProjects/Assignment3/program_3/side.bmp");
    }
    if (backgroundTexture == 0) {
        printf("Warning: Failed to load side.bmp from all attempted paths\n");
    }
...
// Draws the background image as a textured quad behind the scene
void drawBackground() {
    glEnable(GL_TEXTURE_2D);
    glBindTexture(GL_TEXTURE_2D, backgroundTexture);

    // Draw three textured quads for the background walls
    glBegin(GL_QUADS);
        // Back wall
